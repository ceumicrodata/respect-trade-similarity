...
def almost_index(filename):
    ...
    data = data.assign(country_country_total = data.loc[:,["DECLARANT_ISO","PARTNER_ISO"]].apply(lambda df: orszag_orszag_agg[df["DECLARANT_ISO"]][df["PARTNER_ISO"]], $
    ...
    .pipe(lambda x: x.assign(almost_index = x.loc[:,["PRODUCT_NC","prod_share","PARTNER_ISO"]].apply(lambda df: df["prod_share"]*pd.np.log(df["prod_share"]/partner_pro$
    
    data.to_csv("../temp/almost_index_"+filename)
